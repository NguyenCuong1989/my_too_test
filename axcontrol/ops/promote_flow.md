...
- Create adapters/core/apps structure:
  ...
  - `core/loop/control_loop.py`
  - `safety/{kill_switch.py,rate_limit.py}`
  - `apps/{finder.py,safari.py,system_settings.py,notes.py}`
  - `logs/README.md`, `examples/run_*.py`, `scripts/install_permissions.sh`
  - `pyproject.toml`, `.gitignore`, `README.md`
- Move prototypes: `ax_observe.py → adapters/macos_ax/observer.py`, `ax_loop_safe.py → core/loop/control_loop.py`, `llm_strategy_ollama.py → core/decision/llm_strategy.py`, `vision_click_center()` → `adapters/vision/fallback.py`.
